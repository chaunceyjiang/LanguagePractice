...
func (p ProtocolConfig) IPHandler(record map[string]interface{}) {
	ip:=int64(record["DestIp"].(uint64))
	record["DestIp"] = p.int2ip(ip)  // ip的类型发生了变化
	ip =int64(record["SrcIp"].(uint64))
	record["SrcIp"] = p.int2ip(ip)
}
...
func (p ProtocolConfig) NormalizesHandler(record map[string]interface{}) {
	for _, n := range p.Normalizes.Normalize {
		n.NormalizeHandler(record, p.ProtocolPath)
	}
}
...
func (p ProtocolConfig) PrepareHandler(record map[string]interface{}) {
	...
	for _, f = range p.Prepare.Fields {
		record[f.Text] = f.FieldHandler(record, p.ProtocolPath)
	}
}

func (p ProtocolConfig)Process(record map[string]interface{}) map[string]interface{} {
...
func lookup(s ...interface{}) interface{} {
	...
	data,ok:= utils.CsvCache[s[0].(string)]
	if !ok{
