...
def main(dp_endpoint, pipe_num, worker_endpoint, worker_start_no, worker_num, need_do_transform = False):
    ...
    msg_switch = MsgSwitch(context, dp_endpoint, all_worker_endpoint, pipe_num)
    msg_switch.run(need_do_transform)

weekday = ['week_mon', 'week_tue', 'week_wed', 'week_thu', 'week_fri', 'week_sat', 'week_sun']
WEEKDAY = dict(zip(weekday, range(0,7)))

class MsgSwitch(object):
    SendNOTSYNC=0
    SendNOTSYNCCount = 0
    def __init__(self, context, dp_endpoint, all_worker_endpoint, pipe_num):
        self.instance_coll = get_instance_id_collection()
        self.context = context
        self.dp_endpoint = dp_endpoint
        self.all_senders = []
        self.sender_num = len(all_worker_endpoint)
        self.msg_send = MsgSender(context, "tcp://127.0.0.1:3333")
    ...
    def init(self):
        ...
        for worker_endpoint in self.all_worker_endpoint:
            q = Queue()
            MsgSender(worker_endpoint,q).start()
            self.all_senders.append(q)
    ...
    def run(self, need_do_transform):
        try:
    ...
    def transforme_handle(self,record):
        ...
        if time.time() -t1>0.05:
            logger.debug("=======send_msgs %s============="%(time.time() -t1))
    def send_msgs(self,head_info,body_info):
        ...
        if head_info[4] == 0:
            ...
            self.all_senders[int(time.time()*1000000) % self.sender_num].put((head_info[0], sync_head_msg, sync_body_msg, True))
        else:
            # gevent.spawn(self.all_senders[head_info[4] % self.sender_num].send,head_info[0], pack_msg(head_info),pack_msg(body_info),False)
            self.all_senders[head_info[4] % self.sender_num].put((head_info[0], pack_msg(head_info), pack_msg(
                body_info), True))
...
class MsgSender(Process):
    ...
    def __init__(self, send_endpoint,queue):
    ...
    def run(self):
        ...
        sender.connect(self.send_endpoint)
        self.send_multipart = sender.send_multipart
        while True:
            ts,header_msg,body_msg,is_sync = self.queue.get()
            if is_sync:
                now=time.time()
                self.ts=now
                if self.msg_buffer:
