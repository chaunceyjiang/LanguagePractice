...
if __name__=="__main__":
    text1="""
   def get_bucket(self, dtype, topov = None, cap = None, span = None, aggrtype = None,db_name=None, **kwargs):
        # if not db_name:
        #     db_name=get_app_dbName(topov)
        # db = self._get_db(self.DATA_DB, db_name)
        # if dtype == DTYPE_STAT:
        #     return self.get_stat_bucket(db, self, dtype, topov, cap, span, **kwargs)
        # elif dtype == DTYPE_DASHBOARD:
        #     return self.get_dashboard_bucket(db, self, dtype, topov, cap, aggrtype, **kwargs)
        # else:
        #     return GenericMongoBucket(db, self, dtype, topov, cap, **kwargs)
        if check_switch_time():
            timeRange={'start':get_previous_datetime(time.time()),'end':time.time()}
            return self.get_bucket_all(dtype, topov, cap, span,aggrtype,timeRange,**kwargs)
        else:
            return self._get_bucket(dtype, topov, cap, span,aggrtype,db_name,**kwargs)
    """
    
    diff=diff_match_patch()
    # print(diff.diff_main("The cat in the hat.","The bird in the hand."))
